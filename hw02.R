...
.projpackages <- c('GGally','tableone','pander','dplyr','survival');
# name of this script
...
for(xx in aa){
  AA[[xx]] <- update(coxph(formula(xx),dat01),.~.);
}
...
# Part 3 ----
#' #### Part 3. {#part3}
#' **Question 3.1:** Now `AA` is a list of `coxph` objects, containing 
...
#' `r if(identical(unique(c(sapply(models00,sapply,class))),'coxph')) "_You did it! Good job._" else "_Keep trying, you'll get it. Maybe try question 3.2 below if you're stuck._"`
#' 
#' ***
#' 
...
#' (if you correctly answered question 3.1, the code below will show a result in
#' the report).
#+ part4sapply, eval=!is(.test00<-bind_rows(sapply(models00[[1]],glance,simplify=FALSE)),'try-error') && 'p.value.wald' %in% names(.test00)
...
#' (if you correctly answered question 3.1, the code below will show a result in
#' the report).
#+ part4bindrows, eval=!is(.test00,'try-error') && 'p.value.wald' %in% names(.test00)

bind_rows(sapply(models00[[1]],glance,simplify=FALSE),.id='model');

#' To arrange this table in order of increasing `p.value.wald` we can use the 
...
#' (if you correctly answered question 3.1, the code below will show a result in
#' the report).
#+ part4arrange, warning=FALSE, eval=!is(.test00,'try-error') && 'p.value.wald' %in% names(.test00)

arrange_(bind_rows(sapply(models00[[1]],glance,simplify=FALSE),.id='model'),'p.value.wald');
...
#' (if you correctly answered question 3.1, the code below will show a result in
#' the report).
#+ part4padjust, warning=FALSE, eval=!is(.test00,'try-error') && 'p.value.wald' %in% names(.test00)

BB <- arrange_(bind_rows(sapply(models00[[1]],glance,simplify=FALSE),.id='model'),'p.value.wald');
BB[['p.value.wald']];
BB[['p.value']] <- p.adjust(BB[['p.value.wald']]);
BB[['p.value']];
...
#' (if you correctly answered question 3.1, the code below will show a result in
#' the report).
#+ part4mutate, warning=FALSE, eval=!is(.test00,'try-error') && 'p.value.wald' %in% names(.test00)

mutate(arrange_(bind_rows(sapply(models00[[1]],glance,simplify=FALSE),.id='model'),'p.value.wald'),p.value=p.adjust(p.value.wald));
...
#' (if you correctly answered question 3.1, the code below will show a result in
#' the report).
#+ part4pipe, warning=FALSE, eval=!is(.test00,'try-error') && 'p.value.wald' %in% names(.test00)
...
#' (if you correctly answered question 3.1, the code below will show a result in
#' the report).
#+ part4pipearrange, warning=FALSE, eval=!is(.test00,'try-error') && 'p.value.wald' %in% names(.test00)

sapply(models00[[1]],glance,simplify = FALSE) %>% 
  bind_rows(.id='model') %>%
  arrange_('p.value.wald');
...
#+ answer4_1,warning=FALSE,eval=!is(.test00,'try-error') && 'p.value.wald' %in% names(.test00)
# answer4_1----
